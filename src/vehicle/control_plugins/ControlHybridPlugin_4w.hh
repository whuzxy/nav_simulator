...
namespace gazebo
{
  ...
  /// \brief A model plugin for control hybrid
  class ControlHybridPlugin : public ModelPlugin
  {
    /// \brief Constructor.
    public: ControlHybridPlugin();

    /// \brief Destructor.
    public: virtual ~ControlHybridPlugin();
    ...
    private: void OnControlCommand(const control_msgs::Control::ConstPtr &msg);
    ...
    private: void OnmyCmdVel(const ignition::msgs::Any &_msg);
    ...
    /// \brief Private data
    private: std::unique_ptr<ControlHybridPluginPrivate> dataPtr;
  };
}
#endif
