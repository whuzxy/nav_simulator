...
#include "ControlHybridPlugin_4w.hh"
...
namespace gazebo
{
  class ControlHybridPluginPrivate
  {
    /// \enum DirectionType
    ...
    public: double steeringRatio = 0.0826;

    /// \brief Max range of hand steering wheel
    public: double handWheelHigh = 2.5;

    /// \brief Min range of hand steering wheel
    public: double handWheelLow = -2.5;
...
/////////////////////////////////////////////////
ControlHybridPlugin::ControlHybridPlugin()
    : dataPtr(new ControlHybridPluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "ControlHybridPlugin");
  ros::NodeHandle nh;
  this->dataPtr->controlSub = nh.subscribe("/control", 10, &ControlHybridPlugin::OnControlCommand, this);

  this->dataPtr->directionState = ControlHybridPluginPrivate::FORWARD;
...
void ControlHybridPlugin::OnControlCommand(const control_msgs::Control::ConstPtr &msg)
{//ROS_INFO("OnControlCommand");
...
//ROS_INFO("data:  %f\t%f\t%f\n",msg->steer,msg->throttle,msg->brake);
  ...
  this->dataPtr->gasPedalPercent = 1.0;//throttle;
  this->dataPtr->speedPercent = throttle;
//ROS_INFO("data:  %f\t%f\t%f\n",this->dataPtr->handWheelCmd,this->dataPtr->gasPedalPercent,this->dataPtr->brakePedalPercent);
  switch (msg->shift_gears)
  {
    case control_msgs::Control::NEUTRAL:
      this->dataPtr->directionState = ControlHybridPluginPrivate::NEUTRAL;
      break;
    case control_msgs::Control::FORWARD:
      this->dataPtr->directionState = ControlHybridPluginPrivate::FORWARD;
      break;
    case control_msgs::Control::REVERSE:
      this->dataPtr->directionState = ControlHybridPluginPrivate::REVERSE;
      break;
    default:
      break;
  }
...
/////////////////////////////////////////////////
void ControlHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  gzwarn << "ControlHybridPlugin loading params" << std::endl;
  // shortcut to this->dataPtr
  ControlHybridPluginPrivate *dPtr = this->dataPtr.get();

  this->dataPtr->model = _model;
  this->dataPtr->world = this->dataPtr->model->GetWorld();
  auto physicsEngine = this->dataPtr->world->GetPhysicsEngine();
  physicsEngine->SetParam("friction_model", std::string("cone_model"));

  this->dataPtr->gznode = transport::NodePtr(new transport::Node());
  this->dataPtr->gznode->Init();

  this->dataPtr->node.Subscribe("/control/reset",
      &ControlHybridPlugin::OnReset, this);
  this->dataPtr->node.Subscribe("/control/stop",
      &ControlHybridPlugin::OnStop, this);

  this->dataPtr->node.Subscribe("/cmd_vel3", &ControlHybridPlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe("/cmd_vel2", &ControlHybridPlugin::OnmyCmdVel, this);
  this->dataPtr->node.Subscribe("/cmd_gear",
      &ControlHybridPlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe("/cmd_mode",
      &ControlHybridPlugin::OnCmdMode, this);

  this->dataPtr->posePub = this->dataPtr->node.Advertise<ignition::msgs::Pose>(
      "/control/pose");
  this->dataPtr->consolePub =
    this->dataPtr->node.Advertise<ignition::msgs::Double_V>("/control/console");

  std::string chassisLinkName = dPtr->model->GetName() + "::" 
    + _sdf->Get<std::string>("chassis");
  //std::cout << "chassisLinkName "<<chassisLinkName<< std::endl;/////////////
  ...
  //std::cout << "handWheelJointName "<<handWheelJointName<< std::endl;
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&ControlHybridPlugin::Update, this));

  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("/keyboard/keypress",
        &ControlHybridPlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub =
    this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/world_control");

  this->dataPtr->node.Subscribe("/keypress", &ControlHybridPlugin::OnKeyPressIgn,
      this);
}

/////////////////////////////////////////////////
void ControlHybridPlugin::OnCmdVel(const ignition::msgs::Pose &_msg)
{ROS_INFO("cmd_vel");
...
void ControlHybridPlugin::OnmyCmdVel(const ignition::msgs::Any &_msg)
{ROS_INFO("mycmd_vel");
  //this->KeyControl(_msg.int_value());
}

/////////////////////////////////////////////////
void ControlHybridPlugin::OnCmdGear(const ignition::msgs::Int32 &_msg)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  this->dataPtr->directionState =
      static_cast<ControlHybridPluginPrivate::DirectionType>(state);
}

/////////////////////////////////////////////////
void ControlHybridPlugin::OnCmdMode(const ignition::msgs::Boolean &/*_msg*/)
{
  // toggle ev mode
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void ControlHybridPlugin::KeyControlTypeA(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void ControlHybridPlugin::KeyControlTypeB(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  switch (_key)
  {
    ...
    {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != ControlHybridPluginPrivate::REVERSE)
        this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent =
          std::min(this->dataPtr->gasPedalPercent, 1.0);
...
/////////////////////////////////////////////////
void ControlHybridPlugin::KeyControl(const int _key)
{
...
/////////////////////////////////////////////////
void ControlHybridPlugin::OnKeyPress(ConstAnyPtr &_msg)
{ROS_INFO("OnKeyPress");
  this->KeyControl(_msg->int_value());
}

/////////////////////////////////////////////////
void ControlHybridPlugin::OnKeyPressIgn(const ignition::msgs::Any &_msg)
{ROS_INFO("OnKeyPressIgn");
  this->KeyControl(_msg.int_value());
}

/////////////////////////////////////////////////
void ControlHybridPlugin::OnReset(const ignition::msgs::Any & /*_msg*/)
{
...
/////////////////////////////////////////////////
void ControlHybridPlugin::OnStop(const ignition::msgs::Any & /*_msg*/)
{
  ignition::msgs::StringMsg req;
  ignition::msgs::StringMsg rep;
  bool result = false;
  unsigned int timeout = 5000;
  bool executed = this->dataPtr->node.Request("/controlcup/upload",
...
/////////////////////////////////////////////////
void ControlHybridPlugin::Reset()
{
  this->dataPtr->odom = 0;
...
/////////////////////////////////////////////////
void ControlHybridPlugin::Update()
{//ROS_INFO("update");
  // shortcut to this->dataPtr
  ControlHybridPluginPrivate *dPtr = this->dataPtr.get();
  ...
  dPtr->handWheelAngle = dPtr->handWheelJoint->GetAngle ( 0 ).Radian();//Position();
  dPtr->flSteeringAngle = dPtr->flWheelSteeringJoint->GetAngle ( 0 ).Radian();//Position();
  dPtr->frSteeringAngle = dPtr->frWheelSteeringJoint->GetAngle ( 0 ).Radian();//Position();
  ...
  dPtr->chassisLinearVelocity = dPtr->chassisLink->GetWorldLinearVel().Ign();
  ...
  bool neutral = dPtr->directionState == ControlHybridPluginPrivate::NEUTRAL;
  ...
  this->dataPtr->handWheelCmd =
    ignition::math::clamp(this->dataPtr->handWheelCmd,
        ...
        this->dataPtr->maxSteer / this->dataPtr->steeringRatio);
//ROS_INFO("UPDATE  %f",this->dataPtr->handWheelCmd);
  double steerError =
  ...
  this->dataPtr->frWheelSteeringCmd = atan2(tanSteer,
      1 + this->dataPtr->frontTrackWidth/2/this->dataPtr->wheelbaseLength *
      tanSteer);
//  ROS_INFO("hand: %.3f   L: %.3f   R: %.3f",this->dataPtr->handWheelCmd,this->dataPtr->flWheelSteeringCmd,this->dataPtr->frWheelSteeringCmd);
  ...
  creepPercent = ignition::math::clamp(creepPercent, -0.025, 0.1) * 0.0;
  ...
  // of the differential.
  if (fabs(dPtr->flWheelAngularVelocity * dPtr->flWheelRadius) < this->dataPtr->speedPercent * dPtr->maxSpeed &&
      fabs(dPtr->frWheelAngularVelocity * dPtr->frWheelRadius) < this->dataPtr->speedPercent * dPtr->maxSpeed)
  {
    flGasTorque = gasPercent*dPtr->frontTorque * gasMultiplier;
    frGasTorque = gasPercent*dPtr->frontTorque * gasMultiplier;
  }
  if (fabs(dPtr->blWheelAngularVelocity * dPtr->blWheelRadius) < this->dataPtr->speedPercent * dPtr->maxSpeed &&
      fabs(dPtr->brWheelAngularVelocity * dPtr->brWheelRadius) < this->dataPtr->speedPercent * dPtr->maxSpeed)
  {
  ...
  if (this->dataPtr->speedPercent > 0)
...
//   if(!this->dataPtr->speedPercent)
//   {
//     this->dataPtr->flWheelJoint->SetVelocity(0, 0);
//     this->dataPtr->frWheelJoint->SetVelocity(0, 0);
//     this->dataPtr->blWheelJoint->SetVelocity(0, 0);
//     this->dataPtr->brWheelJoint->SetVelocity(0, 0);
//   }
  if(1)
  {
    dPtr->flWheelJoint->SetParam("friction", 0,
      dPtr->flJointFriction + this->dataPtr->brakePedalPercent * dPtr->frontBrakeTorque);
    dPtr->frWheelJoint->SetParam("friction", 0,
      dPtr->frJointFriction + this->dataPtr->brakePedalPercent * dPtr->frontBrakeTorque);
    dPtr->blWheelJoint->SetParam("friction", 0,
      dPtr->blJointFriction + this->dataPtr->brakePedalPercent * dPtr->backBrakeTorque);
    dPtr->brWheelJoint->SetParam("friction", 0,
      dPtr->brJointFriction + this->dataPtr->brakePedalPercent * dPtr->backBrakeTorque);
  }
  double V_ = this->dataPtr->speedPercent * dPtr->maxSpeed;
  double W_ = this->dataPtr->frontTrackWidth;
  double L_ = this->dataPtr->wheelbaseLength;
  double lv = (V_ - W_*tanSteer/2/L_)/dPtr->flWheelRadius;
  double rv = (V_ + W_*tanSteer/2/L_)/dPtr->flWheelRadius;
  if (this->dataPtr->directionState == ControlHybridPluginPrivate::REVERSE)
  {
    lv *= -1.0;rv *= -1.0;
  }
  
  this->dataPtr->blWheelJoint->SetVelocity(0, lv);
  this->dataPtr->brWheelJoint->SetVelocity(0, rv);
//ROS_INFO("L:%.3f  R:%.3f  ,%.3f ",this->dataPtr->blWheelJoint->GetVelocity(0),this->dataPtr->brWheelJoint->GetVelocity(0),tanSteer);
//ROS_INFO("flGas:  %.3f  frGas:  %.3f  Per:  %.3f",flGasTorque,frGasTorque,gasPercent);
  ...
  if ((curTime - this->dataPtr->lastMsgTime) > .5)
  {
    ...
    if (this->dataPtr->directionState == ControlHybridPluginPrivate::FORWARD)
      consoleMsg.add_data(1.0);
    else if (this->dataPtr->directionState == ControlHybridPluginPrivate::REVERSE)
      consoleMsg.add_data(2.0);
    else if (this->dataPtr->directionState == ControlHybridPluginPrivate::NEUTRAL)
...
/////////////////////////////////////////////////
void ControlHybridPlugin::UpdateHandWheelRatio()
{
...
double ControlHybridPlugin::CollisionRadius(physics::CollisionPtr _coll)
{
...
/////////////////////////////////////////////////
double ControlHybridPlugin::GasTorqueMultiplier()
{
...
GZ_REGISTER_MODEL_PLUGIN(ControlHybridPlugin)
