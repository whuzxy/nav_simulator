...
#include "ControlHybridPlugin_2w.hh"
...
namespace gazebo
{
  class ControlHybridPluginPrivate
  {
    ...
    public: double steeringRatio = 0.033;
...
//  dPtr->brJointFriction = dPtr->brWheelJoint->GetParam("friction", 0);
  ...
  ignition::math::Vector3d vec3 = flCenterPos - flCenterPos;
  this->dataPtr->frontTrackWidth = vec3.Length();
  vec3 = flCenterPos - flCenterPos;
  ...
  if ((curTime - this->dataPtr->lastMsgTime) > .5)
  {
    ...
    double linearVel = (2.0 * IGN_PI * this->dataPtr->flWheelRadius) *
      ((this->dataPtr->flWheelAngularVelocity +
        this->dataPtr->flWheelAngularVelocity) * 0.5);
