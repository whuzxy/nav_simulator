...
#include <block_laser/block_laser_16.h>
...
GZ_REGISTER_SENSOR_PLUGIN(GazeboRosBlockLaser2)

////////////////////////////////////////////////////////////////////////////////
// Constructor
GazeboRosBlockLaser2::GazeboRosBlockLaser2()
{
}
...
////////////////////////////////////////////////////////////////////////////////
// Load the controller
void GazeboRosBlockLaser2::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  // load plugin
  ...
  if (!this->parent_ray_sensor_)
    gzthrow("GazeboRosBlockLaser2 controller requires a Ray Sensor as its parent");
  ...
  if (this->topic_name_ != "")
  {
    ...
    ros::AdvertiseOptions ao = ros::AdvertiseOptions::create<sensor_msgs::PointCloud2>(
      this->topic_name_,1,
      boost::bind( &GazeboRosBlockLaser2::LaserConnect,this),
      boost::bind( &GazeboRosBlockLaser2::LaserDisconnect,this), ros::VoidPtr(), &this->laser_queue_);
  ...
  this->callback_laser_queue_thread_ = boost::thread( boost::bind( &GazeboRosBlockLaser2::LaserQueueThread,this ) );

}

////////////////////////////////////////////////////////////////////////////////
// Increment count
void GazeboRosBlockLaser2::LaserConnect()
{
...
////////////////////////////////////////////////////////////////////////////////
// Decrement count
void GazeboRosBlockLaser2::LaserDisconnect()
{
  this->laser_connect_count_--;
...
void GazeboRosBlockLaser2::OnNewLaserScans()
{
  if (this->topic_name_ != "")
  {
...
void GazeboRosBlockLaser2::PutLaserData(common::Time &_updateTime)
{//ROS_INFO("time  start");
  int i, hja, hjb;
  int j, vja, vjb;
  double vb, hb;
...
//  this->cloud_msg_.header.stamp.nsec = _updateTime.nsec;
  ...
  sensor_msgs::PointCloud2Modifier pcd_modifier(this->cloud_msg2_);
  pcd_modifier.setPointCloud2FieldsByString(1, "xyz");
  // convert to flat array shape, we need to reconvert later
  pcd_modifier.resize(rangeCount*verticalRangeCount);
  //ROS_INFO("AAAA %d",verticalRangeCount);
  // this->cloud_msg2_.header.frame_id = this->frame_name_;
  // this->cloud_msg2_.header.stamp.sec = _updateTime.sec;
  // this->cloud_msg2_.header.stamp.nsec = _updateTime.nsec;
  this->cloud_msg2_.width = rangeCount*verticalRangeCount;
  this->cloud_msg2_.height = 1;
  this->cloud_msg2_.point_step = 16;
  this->cloud_msg2_.row_step = this->cloud_msg2_.point_step * this->cloud_msg2_.width;

  this->cloud_msg2_.is_dense = false;

  pcl::PointCloud<pcl::PointXYZI> pcl_cloud;
  //sensor_msgs::PointCloud2Iterator<float> iter_x(this->cloud_msg2_, "x");
  //sensor_msgs::PointCloud2Iterator<float> iter_y(this->cloud_msg2_, "y");
  //sensor_msgs::PointCloud2Iterator<float> iter_z(this->cloud_msg2_, "z");
  ...
  for (j = 0; j<verticalRangeCount; j++)
  {
    ...
    for (i = 0; i<rangeCount; i++)//, ++iter_x, ++iter_y, ++iter_z)
    {
      ...
      if(r>0 && r < 50)
      {        
        pcl_point.x = r * cos(pAngle) * cos(yAngle) + this->GaussianKernel(0,this->gaussian_noise_);
        pcl_point.y = r * cos(pAngle) * sin(yAngle) + this->GaussianKernel(0,this->gaussian_noise_);
        pcl_point.z = r * sin(pAngle) + this->GaussianKernel(0,this->gaussian_noise_);
        pcl_point.intensity = intensity;
      }
      else
      {
        pcl_point.x = pcl_point.y = pcl_point.z = std::numeric_limits<float>::quiet_NaN ();
  ...
  pcl_cloud.height = 1;
  //ROS_INFO("time  4");
  pcl::toROSMsg(pcl_cloud, this->cloud_msg2_);
  //ROS_INFO("time  5");
  this->cloud_msg2_.header.frame_id = this->frame_name_;
  this->cloud_msg2_.header.stamp = ros::Time::now();
  this->cloud_msg2_.is_dense = false;
  //this->cloud_msg2_.header.stamp.sec = _updateTime.sec;
  //this->cloud_msg2_.header.stamp.nsec = _updateTime.nsec;
  // 再将滤波后的点云的数据格式转换为ROS 下的数据格式发布出去
  //sensor_msgs::PointCloud2 output;   //声明的输出的点云的格式
  //sensor_msgs::convertPointCloudToPointCloud2((this->cloud_msg_), output);  
...
// this->lock_.lock();
  ...
  this->pub_.publish(this->cloud_msg2_);
this->lock.unlock();
//  this->lock_.unlock();
//ROS_INFO("time  end");
...
double GazeboRosBlockLaser2::GaussianKernel(double mu,double sigma)
{
...
void GazeboRosBlockLaser2::LaserQueueThread()
{
...
void GazeboRosBlockLaser2::OnStats( const boost::shared_ptr<msgs::WorldStatistics const> &_msg)
{
